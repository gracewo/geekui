<template>
  <Transition name="toast-fade" @after-leave="onAfterLeave">
    ...
    >
      <view
        class="gk-toast-inner"
        :style="{
          'text-align': textAlignCenter ? 'center' : 'left',
          'background-color': bgColor
        }"
      >
        <view v-if="hasIcon" class="gk-toast-icon-wrapper">
          <nut-icon size="20" color="#ffffff" :name="icon"></nut-icon>
        </view>
        <view class="gk-toast-text" v-html="msg"></view>
      </view>
    </view>
  </Transition>
</template>
...
export default create({
  ...
  setup(props) {
    ...
    const toastBodyClass = computed(() => {
      return [
        'gk-toast',
        { 'gk-toast-center': props.center },
        { 'gk-toast-has-icon': hasIcon.value },
        { 'gk-toast-cover': props.cover },
        { 'gk-toast-loading': props.type === 'loading' },
        props.customClass,
        'gk-toast-' + props.size
      ];
    });
